...
wd <- "/Users/avahoffman/Dropbox/Research/Bouteloua_diversity/blue-grama-diversity/genomics_prep"
setwd(wd)

## read in raw snp files
SNPS <- read.delim("SNPs.tab", header=T, sep="\t")
...
process.data.clonesonly <- function(tabdata,
  ...
  ttabdata <- tibble::rownames_to_column(as.data.frame(ttabdata), "ID")
  ...
  genind.data1 <- df2genind(loci,ploidy=2,ind.names=ind,pop=population,sep="")
...
tree.hist<- function(genind.file, ht, wd, ht2, wd2,
                     treefile, treefile2,
                     histfile){
  load(genind.file)
  distgenDISS <- diss.dist(genind.data1, percent = FALSE, mat = FALSE)
  ...
  pdf(file=histfile, width=6, height = 4)
