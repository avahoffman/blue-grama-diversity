...
## save info
write.csv(plot.dat,file="Analysis/genomics_output/DAPC_total.csv")
...
ggsave(file="Analysis/genomics_output/figures/DAPC_total.jpg",height = 3,width=4)
...
summ.dapc.assign
write.csv(summ.dapc.assign, file="Analysis/genomics_output/Population_probability_total.csv")
...
pdf(file="Analysis/genomics_output/figures/Loci_loadings_total.pdf",width=20,height=4)
loadingplot(loci.loadings, axis=2, thres=.004,main=NULL,xlab="Locus")
dev.off()
write.csv(loci.loadings, file="Analysis/genomics_output/Loci_loadings_total.csv")
...
write.csv(long.dat,"Analysis/genomics_output/Structure_plot_data_total.csv")
write.csv(posts,"Analysis/genomics_output/Structure_plot_data_total_wide.csv")
gg2 <- ggplot(data=long.dat, aes(x=Var1,y=value, fill=pop)) +
  ...
  theme_void() +
  theme(legend.position = "none")
ggsave(file="Analysis/genomics_output/figures/structure_total.jpg",height = 1.5,width=10)


finalplot <- grid.arrange(gg1, gg2, nrow=2,heights=c(10, 2))
ggsave(finalplot,file="Analysis/genomics_output/figures/DAPC_and_structure_total.jpg",height = 8,width=10)
