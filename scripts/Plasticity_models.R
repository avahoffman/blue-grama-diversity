...
#save(comp.plasticity.normal, file="plasticity_model_normal.R")
...
Run.vism.plasticity <- function(responsevar,outputname,adapt_delta = 0.8,max_treedepth = 10,iter=10000){
  ...
  write.csv(model_stats1,file=paste("posterior_output_plasticity/",outputname,".csv"))
  ...
  ggsave(post.1, file=paste("posterior_output_plasticity/figures/",outputname,"mean_plasticity.pdf"),height=4,width=4)
  post.2 <- mcmc_intervals(draws, prob = 0.90, prob_outer = 0.95, point_est = "mean",
    ...
    scale_y_discrete(labels=rev(bogr.pops))
  ggsave(post.2, file=paste("posterior_output_plasticity/figures/",outputname,"variance_plasticity.pdf"),height=4,width=4)
  ...
  np1 <- ppc_dens_overlay(temp.data$responsevar, yrep[1:500, ]) + 
    ...
    xlab(outputname)
  ggsave(np1, file=paste("posterior_output_plasticity/predictive_checks/",outputname,"plasticity_normalityplot1.pdf"),height=8,width=8)
  pdf(file=paste("posterior_output_plasticity/predictive_checks/",outputname,"plasticity_normalityplot2.pdf"))
