...
genind.clones <-
  genind.clones[!(indNames(genind.clones) %in% c('Bgedge_9_1_2', 'Bgedge_9_5_1')), ] # These two samples exhibited strange behavior in the tree
...
ggtree(upgma(xdis))
dev.off()
mlg.filter(gen.clones, distance = xdis, algorithm = "average_neighbor") <-
  354 #32
gen.clones
mlg.table(gen.clones) ## this looks correct, notice that at this cutoff, all clones are lumped together.
...
mlg.filter(bg.gen, distance = sample.matrix, algorithm = "farthest_neighbor") <-
  2500#32
