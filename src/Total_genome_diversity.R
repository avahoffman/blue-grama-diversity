...
ggsave(file="genomics_output/figures/DAPC_total.jpg",height = 3,width=4)
...
pdf(file="genomics_output/figures/Loci_loadings_total.pdf",width=20,height=4)
...
gg2 <- ggplot(data=long.dat, aes(x=Var1,y=value, fill=pop)) +
  ...
  theme_void() +
  theme(legend.position = "none")
ggsave(file="genomics_output/figures/structure_total.jpg",height = 1.5,width=10)


finalplot <- grid.arrange(gg1, gg2, nrow=2,heights=c(10, 2))
ggsave(finalplot,file="genomics_output/figures/DAPC_and_structure_total.jpg",height = 8,width=10)
